...
function Navbar() {
  ...
  const itemCounter = useSelector((s) => s.items.itemCounter)

  useEffect(() => {
    updateItemCounter(itemCounter)
  }, [itemCounter])
  ...
  return (
    <nav className="flex items-center justify-between flex-wrap bg-green-600 p-6">
      <div className="flex items-center flex-shrink-0 text-white mr-6">
        ...
        <a href="/" className="font-semibold text-3xl tracking-tight">Macbaren e-store</a>
      </div>
      <span className="tracking-tight text-xl text-white mr-2">Items in the basket: {itemCounter}</span>
      <button className="bg-green-200 hover:bg-green-400 text-gray-800 font-bold py-2 px-4 m-4 rounded">
        <a href="/" className="text-xl text-gray-800 tracking-tight">Go to the basket</a>
      </button>

      <div className="block float-right lg:flex lg:items-right lg:w-auto">
        <div className="float-right mr-6 text-sm lg:flex-grow">
          <span className="tracking-tight text-xl text-white mr-2">Switch currency</span>
          <div className="inline-flex">
            <button className="bg-green-200 hover:bg-green-400 text-gray-800 font-bold py-2 px-4 rounded-l">
              USD
            </button>
            <button className="bg-green-200 hover:bg-green-400 text-gray-800 font-bold py-2 px-4">
              EUR
            </button>
